...
class GetFilteredMoviesUseCase @Inject constructor(private val repository: MovieRepository)  {
    fun getFilteredMovies(sortBy:String?,genreIds: String?,minVote:Float?,maxVote:Float?,releaseDateGte:String?,releaseDatelte:String?,originalLanguage:String?,includeAdult:Boolean?,voteCount:Int?,page:Int?
    ) : Flow<Resource<List<Movies>>> = flow{
        try {
            emit(Resource.Loading())
            val movies=repository.getApplyFilteredMovies(sortBy, genreIds, minVote, maxVote, releaseDateGte, releaseDatelte,originalLanguage,includeAdult,voteCount,page)
